...
• 老婆 摸头 - 消耗老婆币增加亲密度
• 老婆 送礼 - 高消耗高加成亲密度
• 老婆 对话 - 冷却后可增加亲密度并获得老婆币
• 老婆 约会 - 冷却后可消耗老婆币换取更多亲密度
• 老婆 打工 [编号] [加班|远征] - 指定 1 位老婆打工；不写编号默认主老婆
...
💡 提示：`老婆 打工` 每次只会派出 1 位老婆，不会全体出动，也不会随机选人。
💡 提示：部分命令有每日使用次数限制和冷却时间。
💡 `老婆 列表` / `老婆 查` / `老婆 切换` 当前仍未开放，请先使用「查老婆」与「老婆帮助」。
""".strip()
