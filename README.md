...
> ✅ **v3.1.0 — Phase 6 老婆寿命 / 死亡 / 复活系统**
>
> 当前版本在 v3.0.0 四大玩法基础上，新增寿命系统：老婆有寿命值、打工和战斗损失寿命、寿命低会猝死、可花钱复活、跨插件接口支持 impact 联动。
>
> #### 新增核心系统（v3.1.0）
>
> | 系统 | 说明 |
> |---|---|
> | ☠️ **寿命系统** | 每个老婆独立 lifespan 字段；打工/PK 损失寿命；寿命越低猝死概率指数级增长；lifespan=0 强制死亡 |
> | 🕯️ **复活机制** | `老婆 休息` / `养老婆` / `老婆 复活` — 按稀有度×紧急度算价，满血复活死亡老婆 |
> | ⚰️ **死亡状态** | 死亡后老婆**仍在** ownership.json（不删除）；不可打工/不可被牛；可离婚（不返不扣）；可花钱复活 |
> | 🔌 **跨插件接口** | `apply_lifespan_damage_from_impact` — impact 插件按"丁丁尺寸"算 delta 后调用，老婆寿命按规则变化 |
>
> #### v3.0.0 四大玩法
>
> | 系统 | 说明 |
...
- `tests/test_ownership_service.py`：抽老婆/牛老婆/交换/清理完整流程
...
- `老婆 列表 [页码]` / `老婆 查 [@用户] [页码]` 分组入口，等价于 `查老婆`
...
- `老婆 商城` — 查看可购买道具（锁定卡 / 复活药水 / 保护符 / 单抽券 / 十连券）
...
- `老婆 打工 搭档 @用户` — 绑定今日打工搭档，双方都打工时会有额外收益加成
- `老婆 休息` / `养老婆` / `老婆 复活` — 修复/满血复活老婆（按稀有度×紧急度算币）；不写编号时列价格表，写编号直接执行
- `老婆 离婚 <编号>` — 预览离婚返还 + 分家产概率
- `老婆 离婚 <编号> 确认` — 结算离婚

> 💡 老婆编号以 `查老婆` / `老婆 面板` 中显示的序号为准；带 `👑` 的就是当前主老婆，可用 `老婆 切换 <编号>` 手动切换。不写编号时，互动 / PK / 打工 默认使用主老婆。`老婆 打工` 每次只会派出 1 位老婆，不会随机或全体出动。

### 当前仍为占位的分组命令

当前暂无额外占位分组命令；`老婆 列表` / `老婆 查` 已作为 `查老婆` 的分组入口开放。
